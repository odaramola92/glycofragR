...
The package will guide you through Python setup on first use. You need the `glycofrag` Python package installed.
...
# Configure Python environment (do once per session)
set_glycofrag_conda_env("r-glycofrag")
...
**Parameters:**
- `glycan_object`: Result from `glycan_new()`
...
**Parameters:**
- `glycan_object`: Glycan object created via `glycan_new()`
...
#### `peptide_new(sequence, modifications = NULL)`
...
**Example:**
```r
peptide <- peptide_new("LCPDCPLLAPLNDSR", modifications = list(CAM = c(1, 5)))
fragments <- peptide_generate_all_fragments(peptide)
```

---

#### `glycopeptide_new(peptide_sequence, glycan_code, glycosylation_site, glycan_type = "N", modifications = NULL)`
...
peptide <- peptide_new("MPEPTIDE", mod_string = "M:Ox")
...
### Glycan Analysis
- `glycan_new()` - Create glycan object
...
### Peptide Analysis
- `peptide_new()` - Create peptide object
- `peptide_fragments()` - Generate peptide fragments

### Glycopeptide Analysis
- `glycopeptide_new()` - Create glycopeptide object
- `glycopeptide_fragments()` - Generate all fragments
- `glycopeptide_summary()` - Get summary info

### Utilities
- `set_glycofrag_conda_env()` - Configure Python environment
...
```r
# Install glycofrag in your conda environment
reticulate::conda_install("your-env-name", "glycofrag")

# Then reconfigure
glycofragR::set_glycofrag_conda_env("your-env-name")
```
